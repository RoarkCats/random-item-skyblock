...
execute align xyz run summon area_effect_cloud ~0.5 ~0.85 ~0.5 {Tags:["ris.multiplier"],Duration:2147000000,CustomName:'{"text":"Item Multiplier"}'}

playsound block.stone.place block @a ~ ~ ~ 1 0.8
kill @s[type=item]

execute on origin run advancement grant @s only ris:special/customized/commander chain_command_block