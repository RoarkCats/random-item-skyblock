# Item Giver
execute if score special_items ris.settings matches 1 unless score creator_heads ris.special_item_settings matches 0 if score same_items ris.settings matches 1 if score time ris.timer = half_delay ris.timer run loot give @a[tag=!ris.non_receiver,gamemode=!spectator,predicate=ris:creator_heads/phoenixsc] loot ris:random_item
execute if score special_items ris.settings matches 1 unless score creator_heads ris.special_item_settings matches 0 if score same_items ris.settings matches 0 if score time ris.timer = half_delay ris.timer as @a[predicate=ris:creator_heads/phoenixsc] run loot give @s[tag=!ris.non_receiver,gamemode=!spectator] loot ris:random_item

execute unless score item_giver ris.settings matches 0 if score same_items ris.settings matches 1 if score time ris.timer >= delay ris.timer as @r run loot give @a[tag=!ris.non_receiver,gamemode=!spectator] loot ris:random_item
execute unless score item_giver ris.settings matches 0 if score same_items ris.settings matches 0 if score time ris.timer >= delay ris.timer as @a run loot give @s[tag=!ris.non_receiver,gamemode=!spectator] loot ris:random_item
