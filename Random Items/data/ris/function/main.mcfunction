# Item Giver
execute if predicate ris:creator_heads/enabled if score same_items ris.settings matches 1 if score time ris.timer = half_delay ris.timer run loot give @a[tag=!ris.non_receiver,gamemode=!spectator,nbt=!{Health:0f},predicate=ris:creator_heads/phoenixsc] loot ris:random_item
execute if predicate ris:creator_heads/enabled if score same_items ris.settings matches 0 if score time ris.timer = half_delay ris.timer as @a[predicate=ris:creator_heads/phoenixsc] run loot give @s[tag=!ris.non_receiver,gamemode=!spectator,nbt=!{Health:0f}] loot ris:random_item
