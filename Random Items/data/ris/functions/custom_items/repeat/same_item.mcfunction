...
execute as @e[tag=ris.generator,tag=ris.origin_item] at @s positioned ~ ~0.4 ~ run data modify storage ris generator.Item set from entity @e[type=item,nbt={Age:0s},limit=1,sort=nearest] Item

execute as @e[tag=!ris.gen_off,tag=ris.generator,tag=!ris.origin_item] at @s positioned ~ ~0.4 ~ run summon item ~ ~ ~ {Tags:["ris.spawned_item","ris.blank_item"],Item:{id:"minecraft:stone",Count:1b}}
